...
function L_p1238__dB = model_p1238(d__km, param_los = 1, param_env = 1)
  globals
  coeff_row = 2 * param_env - param_los;
    ...
    if(warnings)
      ...
      if(d__km < loss_freqs(coeff_row,3)) printf("Warning: distance lower than model minimum %d", loss_freqs(coeff_row,3)), disp(" m") endif
      if(d__km > loss_freqs(coeff_row,4)) printf("Warning: distance exceeds model maximum %d", loss_freqs(coeff_row,4)), disp(" m") endif
    endif
    ...
    L_p1238__dB = -(10 * alpha .* log10(d__km) + beeta + 10 * gamma * log10(f) + norm_inv(q) * sigma);
endfunction