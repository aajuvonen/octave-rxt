...
function path_loss_p1238 = model_p1238(param_dist,param_los = 1,param_env = 1)
  ...
  coeff_row = 2 * param_env - param_los;
    ...
    if(warnings)
      ...
      if(param_dist < loss_freqs(coeff_row,3)) printf("Warning: distance lower than model minimum %d", loss_freqs(coeff_row,3)), disp(" m") endif
      if(param_dist > loss_freqs(coeff_row,4)) printf("Warning: distance exceeds model maximum %d", loss_freqs(coeff_row,4)), disp(" m") endif
    endif
