...
%                urban factor  num.  (0 / 1)      0 = small/medium city (buildings <= 15 m), 1 = large city (buildings > 15 m)
%                   city size  num.  (0...1)      0 = open area, 0.5 = suburban area, 1 = urban area
%                   buildings  [%]   (3...50)     Percentage of buildings on the land (nominally 15.849 %)
%                   frequency  [MHz] (100...3000)
%
% Output:           path loss  [dB]

function path_loss_hata_mod = model_hata_mod(param_dist,param_h1,param_h2,param_u = 0,param_u_r = 0,param_b = 15.849)
  globals
  ...
  if(warnings)
    ...
    L_ccir += a_x;                                                       % Eq. (7.11)
    path_loss_hata_mod = -(L_ccir + S_o - S_ks + B_o);                   % Eq. (7.23)
endfunction